...
The Vitable Connect Ruby library provides convenient access to the Vitable Connect REST API from any Ruby 3.2.0+ application. It ships with comprehensive types & docstrings in Yard, RBS, and RBI – [see below](https://github.com/Vitable-Inc/vitable-connect-ruby#Sorbet) for usage with Sorbet. The standard library's `net/http` is used as the HTTP transport, with connection pooling via the `connection_pool` gem.
...
```ruby
page = vitable_connect.employees.list_enrollments

# Fetch single item from page.
employee = page.data[0]
puts(employee.id)

# Automatically fetches more pages as needed.
page.auto_paging_each do |employee|
  puts(employee.id)
end
```
...
```ruby
# :Member
puts(VitableConnect::AuthCompleteProfileParams::UserType::MEMBER)

# Revealed type: `T.all(VitableConnect::AuthCompleteProfileParams::UserType, Symbol)`
T.reveal_type(VitableConnect::AuthCompleteProfileParams::UserType::MEMBER)
```
...
vitable_connect.auth.complete_profile(
  user_type: VitableConnect::AuthCompleteProfileParams::UserType::MEMBER,
  # …
)
...
See [the contributing documentation](https://github.com/Vitable-Inc/vitable-connect-ruby/tree/main/CONTRIBUTING.md).
